...
class Game {
  ...
  getElapsedTime() {
    ...
    if (this.endTime) {
      return Math.floor((this.endTime - this.startTime) / 1);
    }

    // if the game is on pause, exclude the paused time
    if (this.pauseTime) {
      return Math.floor((this.pauseTime - this.startTime) / 1);
    }

    // elapsed time while the game is played
    return Math.floor((new Date().getTime() - this.startTime) / 1);
  }
